...
function JobCard({ job, cvData }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-blue-300 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 flex flex-col gap-3">
      ...
      {matchBadge}
      <Link to={`/jobs/${job._id}`} className="block mt-auto w-full text-center text-sm font-medium text-blue-600 border border-blue-200 rounded-xl py-2 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors bg-blue-50">View & Apply</Link>
    </div>
  );
}
